#!/usr/bin/env python3
"""
Experiment 012 — Score COL4A5 stratified variants with AlphaGenome live API.

Reads outputs/_col4a5_stratified.tsv (produced by _col4a5_extract.py),
...
Saves:
  outputs/cross_disease_col4a5_raw.csv
  outputs/cross_disease_col4a5_metrics.csv
  outputs/cross_disease_col4a5_filtered_raw.csv
  outputs/cross_disease_col4a5_filtered_metrics.csv

Run via:
    bash scripts/_run_with_key.sh scripts/_col4a5_score.py
"""
...
# Unfiltered caps (mirror KCNQ2 / SCN1A pattern: 200/350)
N_POS_CAP_UNFILT = 200
N_NEG_CAP_UNFILT = 350

# Filtered caps: we have 176 pathogenic+splice and 420 benign+intronic
N_POS_CAP_FILT = 176
N_NEG_CAP_FILT = 200
...
def run_one(dna_model, bench: pd.DataFrame, label: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for scorer in SCORER_NAMES:
        m = compute_metrics(raw_df, scorer, gene="COL4A5")
        metrics.append(m)
...
def main() -> int:
    ...
    parser.add_argument("--input", default="outputs/_col4a5_stratified.tsv")
    parser.add_argument("--raw-out", default="outputs/cross_disease_col4a5_raw.csv")
    parser.add_argument("--metrics-out",
                        default="outputs/cross_disease_col4a5_metrics.csv")
    parser.add_argument("--filtered-raw-out",
                        default="outputs/cross_disease_col4a5_filtered_raw.csv")
    parser.add_argument("--filtered-metrics-out",
                        default="outputs/cross_disease_col4a5_filtered_metrics.csv")
    ...
    print("\nSmoke test...")
    try:
        v = genome.Variant(chromosome="chrX", position=108_500_000,
    ...
    if not args.skip_unfiltered:
        print("\nBuilding COL4A5 unfiltered benchmark...")
    ...
    if not args.skip_filtered:
        print("\nBuilding COL4A5 filtered (apples-to-apples) benchmark...")
